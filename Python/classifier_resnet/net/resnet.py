...
class ResNet18(nn.Module):  # 构建ResNet18模型
    def __init__(self, num_classes=2):
        ...
        # [b, 256, 7, 7]
        self.outlayer = nn.Linear(256 * 3 * 3, num_classes)  # 256*3*3:由输入张量self.blk4的形状决定
...
class ResNet_Feature(nn.Module):  # 构建ResNet18模型
    def __init__(self, num_classes=2):
        ...
        self.blk4 = ResBlk(128, 256, stride=2)
        # [b, 256, 7, 7]
        self.outlayer = nn.Linear(256 * 3 * 3+8, num_classes)  # 256*3*3:由输入张量self.blk4的形状决定
        # 3*3必须经过图片->conv1->blk1->...blkn(具体步骤参考函数forward),一步步计算

    def forward(self, x,f_add):
        """
        :完成正向网络的串联
        :param x:
        :return:
        ...
        x = torch.hstack((x,f_add*self.feature_w))
...
if __name__ == '__main__':
    ...
    cnn = ResNet18(num_classes=2) # 导入VGG19模型并且将数据转到显存中
    # gpu_tracker.track()
