...
class DecodeBoxScript(nn.Module):
    ...
    def decode_box(self, inputs_1):
        ...
        # scaled_anchors = scaled_anchors_list
        ...
        prediction = inputs_1.view(batch_size, len(self.anchors_mask[i]),
                                self.bbox_attrs, input_height, input_width).permute(0, 1, 3, 4, 2).contiguous().to(dpara_device)
        ...
        #   将输出结果归一化成小数的形式
        # ----------------------------------------------------------#
        _scale = torch.tensor([input_width, input_height, input_width, input_height],dtype=torch.float32).to(dpara_device)
        output = torch.cat((pred_boxes.view(batch_size, -1, 4) / _scale,
                            conf.view(batch_size, -1, 1), pred_cls.view(batch_size, -1, self.num_classes)), -1)
        return output


        # for i, input in enumerate([inputs_1,inputs_2,inputs_3]):
        #     scaled_anchors_list = []
        #     # -----------------------------------------------#
        #     #   输入的input一共有三个，他们的shape分别是
        #     #   batch_size, 255, 13, 13
        ...
        #     #   batch_size,3,13,13
        #     # ----------------------------------------------------------#
        #     # grid_x1 = torch.linspace(0, input_width - 1, input_width)
        #     # grid_x2 = grid_x1.repeat(input_height, 1)
        #     # grid_x = grid_x2.repeat(batch_size * len(self.anchors_mask[i]), 1, 1).view(x.shape)
        #     # grid_x = torch.linspace(0, input_width - 1, input_width).repeat(input_height, 1).repeat(
        ...
        #
        #     grid_x = torch.linspace(0, input_width - 1, input_width).to(dtype=torch.float32).repeat(input_height, 1).repeat(batch_size * len(self.anchors_mask[i]), 1, 1).view(x.shape)
        #     grid_y = torch.linspace(0, input_height - 1, input_height).to(dtype=torch.float32).repeat(input_width, 1).t().repeat(batch_size * len(self.anchors_mask[i]), 1, 1).view(y.shape)
        #     # grid_x = torch.linspace(0, input_width - 1, input_width).repeat(input_height, 1).repeat(3, 1, 1).view(x.shape)
        #     # grid_y = torch.linspace(0, input_height - 1, input_height).repeat(input_width, 1).t().repeat(3, 1, 1).view(y.shape)
        #     # ----------------------------------------------------------#
        #     #   按照网格格式生成先验框的宽高
        #     #   batch_size,3,13,13
        #     # ----------------------------------------------------------#
        ...
        #     #   将输出结果归一化成小数的形式
        #     # ----------------------------------------------------------#
        #     _scale = torch.tensor([input_width, input_height, input_width, input_height]).to(dpara_device)
        #     output = torch.cat((pred_boxes.view(batch_size, -1, 4) / _scale,
        #                         conf.view(batch_size, -1, 1), pred_cls.view(batch_size, -1, self.num_classes)), -1)
        #     outputs.append(output.data)
    ...
    def non_max_suppression(self, prediction, num_classes, input_shape, image_shape, conf_thres,
        ...
        #统计每个图中图形
        output_c = torch.zeros(len(unique_labels),7)
        ...
        for unique_labels_id, c in enumerate(unique_labels):
            ...
            ### class, keep_anchors, 7(x1, y1, x2, y2, obj_conf, class_conf, class_pred)
            # output_c[unique_labels_id:unique_labels_id+1,None,:] = max_detections
        ...
        output_c[:, :4] = self.yolo_correct_boxes(box_xy, box_wh, input_shape, image_shape)
...
#         self.anchors_mask = anchors_mask
#
#     def decode_box(self, inputs_1,inputs_2):
#         dpara_device = inputs_1.device
#         outputs = []
#
#         for i, input in enumerate([inputs_1,inputs_2]):
#             scaled_anchors_list = []
#             # -----------------------------------------------#
#             #   输入的input一共有三个，他们的shape分别是
#             #   batch_size, 255, 13, 13
...
#                 s_w = (anchor_size[0] / stride_w).unsqueeze(0).unsqueeze(1)
#                 s_h = (anchor_size[1] / stride_h).unsqueeze(0).unsqueeze(1)
#                 # s_w = anchor_size[0]  / stride_w
#                 # s_h = anchor_size[1] / stride_h
#                 # scaled_anchors_list.append((s_w, s_h))
#
#                 scaled_anchors_list.append(torch.cat((s_w,s_h),dim=1))
#             scaled_anchors = torch.cat(scaled_anchors_list, dim=0)
#             # scaled_anchors = scaled_anchors_list
...
#
#         boxes = torch.cat(([box_mins[:, 0:1], box_mins[:, 1:2], box_maxes[:, 0:1], box_maxes[:, 1:2]]),dim=1)
#         boxes = boxes *torch.cat([image_shape, image_shape], dim=0)
#         # boxes *= np.concatenate([image_shape, image_shape], axis=-1)
#         return boxes
#
#     def non_max_suppression(self, prediction, num_classes, input_shape, image_shape, conf_thres,
...
#         # output = torch.zeros((len(prediction),7))
#     # for i, image_pred in enumerate(prediction):
#         image_pred = prediction[0]
#         # ----------------------------------------------------------#
#         #   对种类预测部分取max。
...
#             #     ious = bbox_iou(max_detections[-1][:, :4], detections_class[1:][:, :4])
#             #     detections_class = detections_class[1:][ious < nms_thres]
#             # 堆叠
...
#             else:
#                 output_c = torch.cat((output_c, max_detections),dim=0)
#         if torch.sum(output_c)>0:
#             # output[i] = output[i].cpu().numpy()
#             box_xy = (output_c[:, 0:2] + output_c[:, 2:4]) / 2
#             box_wh= output_c[:, 2:4] - output_c[:, 0:2]
#             output_c[:, :4] = self.yolo_correct_boxes(box_xy, box_wh, input_shape, image_shape)
#             # output[i]= self.yolo_correct_boxes(box_xy, box_wh, input_shape, image_shape, letterbox_image)
#         return output_c
