...
def load_data_make_blobs(input_shape):

    img_info = r'G:\DefectDataCenter\ParseData\Detection\COT_Raw\raw_data\Config\ImgBoxes_train.txt'
    w,h = input_shape
    ...
    for ln in lns:
        box_infs = ln.split(" ")
        box = [[int(y) for y in  x.split(',')[:-1]] for x in box_infs[1:]]
        box_wh = [[(p[2]-p[0])/w,(p[3]-p[1])/h] for p in box]
        boxes_wh.extend(box_wh)
    return np.array(boxes_wh)


if __name__ == '__main__':
    input_shape = [1,1]  ##input_shape = [2688,424]
    X = load_data_make_blobs(input_shape)  # 获取模拟数据和聚类数量
    s = time.time()
    k = 6
