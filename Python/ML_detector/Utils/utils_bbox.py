...
#             anchor_w = anchor_w.repeat(batch_size, 1).repeat(1, 1, input_height * input_width).view(w.shape).to(device)
#             anchor_h = anchor_h.repeat(batch_size, 1).repeat(1, 1, input_height * input_width).view(h.shape).to(device)
#
#
#
#             # ----------------------------------------------------------#
...
#                 _scale = torch.tensor([input_width, input_height, input_width, input_height]).to(device)
#             else:
#                 _scale = torch.stack((input_width, input_height, input_width, input_height), dim=0).to(device)
#
#             output = torch.cat((pred_boxes.view(batch_size, -1, 4) / _scale,
#                                 conf.view(batch_size, -1, 1), pred_cls.view(batch_size, -1, self.num_classes)), -1)
#             outputs.append(output.data)
...
class DecodeBoxScript(nn.Module):
    ...
    def decode_box(self, inputs_1,i):
        ...
        if isinstance(input_width, int):
            ...
            _scale = torch.stack((input_width, input_height, input_width, input_height), dim=0).to(dpara_device)
    ...
    def yolo_correct_boxes(self, box_xy, box_wh, image_shape):
        ...
        b = torch.ones(2).to(boxes.device)
        b[0] ,b[1]= image_shape[0], image_shape[1]

        box_scale = b.repeat(2)
        boxes = boxes*box_scale

        # boxes_t = torch.zeros_like(boxes)
        # boxes_t[:,0],boxes_t[:,1] = boxes[:,1],boxes[:,0]
        # boxes_t[:, 2], boxes_t[:, 3] = boxes[:, 3], boxes[:, 2]
        return boxes
